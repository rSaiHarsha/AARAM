...
class RAGEngine:
    ...
    def process_file(self, file_name, file_content, collection_name=None, start_page=None, end_page=None):
        ...
        # 1. Handle PDF files using PyMuPDF / pymupdf4llm
        if file_name.lower().endswith('.pdf'):
            try:
                ...
                raise ImportError(
                    ...
                    "Please install PyMuPDF and pymupdf4llm: pip install pymupdf pymupdf4llm"
                )
            ...
            try:
                ...
                # Determine page range (1-indexed input converted to 0-indexed for pymupdf)
                s_page = 1 if start_page is None else int(start_page)
                e_page = total_pages if end_page is None else int(end_page)
                
                # Boundary checks
                s_page = max(1, min(s_page, total_pages))
                e_page = max(s_page, min(e_page, total_pages))
                
                pages_0_indexed = list(range(s_page - 1, e_page))
                
                # Convert PDF to markdown page chunks for specific pages
                pages = pymupdf4llm.to_markdown(doc, pages=pages_0_indexed, page_chunks=True)
                
                for i, page in enumerate(pages):
                    actual_page_num = pages_0_indexed[i] + 1
                    ...
                    raw_chunk = {
                        "id": str(uuid.uuid4()),
                        "title": f"{file_name} (Page {actual_page_num})",
                        "text": text_content,
                        "source": file_name,
                    ...
                    # Split oversized chunks if necessary
                    split_chunks = self._split_oversized_chunk(raw_chunk, max_chars=1800)
                    for sc in split_chunks:
                        sc["text"] = f"Document: {file_name}\nPage: {actual_page_num}\n\n{sc['text']}"
                        self.documents.append(sc)
                return
            except Exception as e:
                print(f"[RAGEngine] Failed to process PDF file {file_name}: {e}", flush=True)
                return

        # 2. Handle Excel files directly from bytes
        ...
        if file_name.lower().endswith(".json"):
            ...
            try:
                ...
                print(f"[RAGEngine] JSON decode failed for {file_name}: {e}. Falling back to text decode.", flush=True)
                try:
    ...
    def train_engine(self, progress_callback, collection_name=None):
        ...
        from Analysis.quality_analyser import _StreamlitThreadPoolExecutor

        def process_doc(idx, text):
            return idx, self._safe_get_embedding(text)

        completed = 0
        with _StreamlitThreadPoolExecutor(max_workers=2) as executor:
